...
- 将扩展与 package 元数据提升到 `1.0.9`，并重新生成本地 Chrome/Edge 商店上传包。
- 更新 README 中的工具栏 Tools 入口、轻量设置面板和当前 14 套主题说明。

### 修复

- 确保窄屏下设置面板不会超出视口，并让聚焦在设置/搜索输入框时的 Escape 先失焦再处理面板关闭。
- 统一可恢复 URL 校验，保存标签、保存会话、恢复会话和从存储打开 URL 时只允许 `http:`、`https:` 和 `file:` 地址。

## 1.0.8 - 2026-05-16

### 变更

- 将扩展与 package 元数据提升到 `1.0.8`，用于下一次 Chrome/Edge 商店上传。
