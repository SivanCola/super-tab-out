...
- 将扩展与 package 元数据提升到 `1.0.5`，并重新生成 Chrome/Edge 商店上传包。

## 1.0.4 - 2026-05-14

### 修复

- 将 `app.js` 加回 Chrome/Edge 发布包白名单，确保商店包中的新标签页仪表盘可以启动、读取打开的标签页，并正确渲染工具 dock。
- 将 `app.js` 纳入 ZIP 产物校验，避免发布包漏掉运行时入口脚本时仍然通过校验。

## 1.0.3 - 2026-05-12

### 变更

- 将受限的本地二维码编码器替换为 vendored MIT 二维码生成库，仍然完全本地运行并支持标准二维码版本。
- 新增 Chrome/Edge 白名单打包，移除发布包中的源素材和不可达 Side Panel 代码。
- 将扩展与 package 元数据提升到 `1.0.3`，并重新生成 Chrome/Edge 商店包。
