...
Super Tab Out 会替换默认新标签页，把你打开的标签集中成一个清爽的可视化面板。它支持按域名分组，也支持按 Chrome 原生 Tab Groups 分组；同时提供固定标签保护、快速搜索、隐私模式、中英文界面和更丰富的主题选择。
...
- **快速搜索标签**：按 `/` 后可实时过滤卡片、标签、URL 和分组名。
- **适合屏幕共享的隐私模式**：点击锁图标或按 `Esc`，把标签面板隐藏成简洁的时钟/搜索屏幕。
...
隐私模式会隐藏标签面板，显示简洁的时钟、日期和搜索界面，适合屏幕共享或录屏前使用。你可以自定义是否显示：
...
隐私模式里有一个可选的 Google 搜索表单。如果你在隐私设置中关闭搜索框，该表单不会显示。
...
本分叉在此基础上加入了安全加固、Manifest V3 更新、跨浏览器新标签识别、固定标签保护、Chrome Tab Groups 视图、中英文界面、扩展主题、隐私模式、搜索、favicon 缓存以及本地优先的稍后再看流程。
